...
patch(ListController.prototype, {
    ...
    get display() {
        ...
        if (!this.props.readonly && this.activeActions.edit && res.controlPanel) {
            ...
            const setMode = (mode) => {
                this.editable = mode === 'edit' ? initialEditable : false;
                this.model.multiEdit = mode === 'edit' ? true : initialMultiEdit;
                browser.localStorage.setItem(this.getModeStorageKey(), mode);
                this.model.notify();
            };
            res.controlPanel.modeSwitch = {
                currentMode: modeEntries.find(
                    (e) => e.mode === (this.editable ? 'edit' : 'read'),
                ),
