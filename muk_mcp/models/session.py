...
class MCPSession(models.Model):
    ...
    def init(self):
        super().init()
        if not tools.index_exists(self.env.cr, 'muk_mcp_session_active_session_idx'):
            self.env.cr.execute(
                f'CREATE INDEX "muk_mcp_session_active_session_idx" '
                f'ON "{self._table}" (session_id, user_id) WHERE active IS TRUE'
            )
