...
class MCPNotification(models.Model):
    ...
    def init(self):
        super().init()
        if not tools.index_exists(self.env.cr, 'muk_mcp_notification_undelivered_idx'):
            self.env.cr.execute(
                f'CREATE INDEX "muk_mcp_notification_undelivered_idx" '
                f'ON "{self._table}" (session_id, id) WHERE delivered IS NOT TRUE'
            )
