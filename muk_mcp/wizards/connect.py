...
class Connect(models.TransientModel):
    """Wizard presenting MCP connection snippets for supported AI clients."""
    ...
    def action_generate_key(self) -> dict[str, Any]:
        """Generate a playground bearer key and reopen the wizard showing it."""
        self.ensure_one()
    ...
    def _compute_mcp_url(self) -> None:
        """Compute the absolute /mcp endpoint URL from configuration."""
        for record in self:
            record.mcp_url = (record.get_base_url() or '').rstrip('/') + '/mcp'

    @api.depends('mcp_url', 'bearer_key')
    def _compute_snippets(self) -> None:
        """Build the per-client config snippets shown in the connect wizard."""
        for record in self:
