...
class MCPHttpCase(HttpCase):
    ...
    def mcp_ping(self, request_id: int = 1) -> dict:
    ...
    def mcp_get(self, **kwargs: Any) -> Response:
        ...
        return self.url_open('/mcp', headers=self.mcp_headers(**kwargs))

    def mcp_delete(self, **kwargs: Any) -> Response:
        """Send a ``DELETE /mcp`` request with the given MCP headers.

        Issued straight through the test opener: ``url_open`` only ever sends
        ``GET``, ``POST`` or ``HEAD``, so it cannot reach the terminate route.
        """
        kwargs.setdefault('content_type', None)
