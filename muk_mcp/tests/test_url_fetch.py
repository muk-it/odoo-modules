...
class FakeResponse:
    """Stand in for a streamed urllib3 response."""
    ...
    def __init__(
        ...
        """Store the status, body, and headers the fetch will read."""
        self.status = status
        self.headers = headers or {}
        self.body = body

    def stream(self, _chunk_size: int) -> Iterator[bytes]:
        """Yield the whole body as a single chunk."""
        yield self.body

    def release_conn(self) -> None:
        """Accept the connection release the fetch performs."""


@tagged('post_install', '-at_install')
class TestUrlFetch(common.TransactionCase):
    """Verify the set_binary_from_url tool and its SSRF-guarded download."""
    ...
    @staticmethod
    def _getaddrinfo(host: str, *_args, **_kwargs) -> list[tuple]:
        """Resolve IP literals to themselves and any hostname to a public address."""
        try:
    ...
    def _mock_network(self, *responses: FakeResponse) -> MagicMock:
        """Serve ``responses`` in order from a mocked, IP-pinned connection pool."""
        pool = MagicMock()
        ...
        self.startPatcher(patch.object(url_fetch.urllib3, 'HTTPSConnectionPool', pool))
        return pool

    def _set_image(self, url: str) -> Any:
        """Set the test partner's image_1920 from ``url`` through the tool."""
        return self._call(
            'set_binary_from_url',
            {
                'model': 'res.partner',
    ...
    def test_set_binary_from_url(self):
        pool = self._mock_network(
            FakeResponse(200, PNG, {'Content-Type': 'image/png'}),
        )
    ...
    def test_rejects_redirect_to_private_address(self):
        pool = self._mock_network(
            FakeResponse(302, headers={'Location': 'https://192.168.0.1/image.png'}),
        )
        with self.assertRaisesRegex(UserError, 'not publicly routable'):
