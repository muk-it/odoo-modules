...
class TestVersionNegotiationHttp(MCPHttpCase):
    ...
    def _initialize(self, protocol_version=None):
        """Run ``initialize`` and return the raw response, body and session id."""
    ...
    def _stored_version(self, session_id):
    ...
    def test_the_retired_methods_are_gone_from_the_stateless_revision(self):
        for method in (
            ...
            self.assertEqual(
                ...
                404,
                '%s should be unroutable on the stateless revision' % method,
            )
    ...
    def test_the_retired_methods_still_serve_the_stateful_revisions(self):
        ...
        for method in ('ping', 'logging/setLevel'):
            ...
            self.assertNotIn('error', body, '%s should still be served' % method)
