...
export class AISessionBox extends Component {
    static template = 'muk_ai_automation.AISessionBox';
    static props = {
        threadModel: String,
    ...
    /**
     * Format an ISO datetime string as a compact "YYYY-MM-DD HH:MM" label.
     * @param {string} value the ISO datetime to format
     * @returns {string} the trimmed label, or an empty string when falsy
     */
    relativeName(value) {
        if (!value) {
            return '';
