...
test('clicking the icon reveals the text value in a popover', async () => {
    ...
    await contains('.o_list_table tbody tr:nth-child(1) .oi[data-icon="book"]').click();
    await animationFrame();
    expect('.o_popover').toHaveCount(1);
    expect(queryText('.o_popover')).toBe('Has value');
    expect('.o_popover .mk_text_value_popover').toHaveCount(1);
    expect('.o_popover .o-tooltip').toHaveCount(0);
});

test('the icon carries the field label as its title', async () => {
    await mountView({ type: 'list', resModel: 'product', arch: ARCH });
    expect('.o_list_table tbody tr:nth-child(1) .oi[data-icon="book"]').toHaveAttribute(
        'title',
        'Description',
    );
});
