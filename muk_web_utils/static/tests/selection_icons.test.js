        ...
        arch: listArch({
            body: `
                <field
                    ...
                    nolabel="1"
                    options="{'icons': {'ready': 'check'}, 'defaultIcon': 'question_mark'}"
                />
            `,
        }),
    });
    expect('.o_list_table tbody tr:nth-child(1) i.oi[data-icon="check"]').toHaveCount(
        1,
    );
    expect(
        '.o_list_table tbody tr:nth-child(2) i.oi[data-icon="question_mark"]',
    ).toHaveCount(1);
});
