...
class TestEncoder(TransactionCase):
    ...
    def test_record_encoder_datetime(self):
        result = json.dumps({'d': datetime(2026, 4, 16, 12, 0, 0)}, cls=RecordEncoder)
        self.assertIn('2026-04-16', result)

    def test_record_encoder_date(self):
        result = json.dumps({'d': date(2026, 4, 16)}, cls=RecordEncoder)
        self.assertIn('2026-04-16', result)
