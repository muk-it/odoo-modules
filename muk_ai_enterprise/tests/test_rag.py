...
class TestRag(BridgeTestCommon):
    ...
    def _make_agent_with_source(self):
        ee_agent = self.env['ai.agent'].sudo().search([], limit=1)
        if not ee_agent:
            ee_agent = self.env['ai.agent'].sudo().create({
        ...
        source = self.env['ai.agent.source'].sudo().create({
    ...
    def test_rag_block_added_when_chunks_returned(self):
        ...
        session.write({'conversation': [{
            'role': 'user',
            'content': [{'type': 'input_text', 'text': 'tell me about frobnication'}],
        }]})
        chunk = self.env['ai.embedding'].sudo().new({
            'attachment_id': attachment.id,
