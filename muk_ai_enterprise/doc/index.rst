...
chat's view context. The Enterprise data/model layer is never
modified. Its systray button is folded into the MuK AI dropdown as
**Open Odoo AI** (``Alt+Shift+R``) — one AI icon, not two. Everything
else (Discuss ``ai_chat`` channels, chatter / composer buttons,
command-palette ``Ask AI``, ``ai.agent`` replies, ``LLMApiService``,
``ai_fields`` cron, optional ``ai_*`` satellites) keeps its existing
behaviour.
