...
class MCPTool(models.Model):
    ...
    def _resolve_ee_action(self, tool_name):
        ...
        if tech.startswith('action_') and tech[7:].isdigit():
            recs = self.env['ir.actions.server'].sudo().browse(
                int(tech[7:])
            )
        ...
        ).filtered('use_in_ai').exists().sorted('id')[:1]

    @api.model
    def _resolve_ee_record(self, env):
        session = env['muk_ai.session'].sudo().browse(
            env.context.get('muk_mcp_session_id') or 0
        )
        vc = (
        ...
        if vc.get('kind') != 'record':
            return env.user
        if (
            not (model_name := vc.get('model')) or
            not isinstance(res_id := vc.get('id'), int) or
            env.registry.get(model_name) is None
        ):
            return env.user
    ...
    def _call_ee_action(self, tool_name, arguments, env, enforce_scope=None):
        if enforce_scope == 'read':
            raise UserError(_(
                "Read-only agents cannot call EE action tools (%s).", tool_name,
            ))
        ...
        try:
            ...
            raise UserError(_(
                "EE action %(name)s failed: %(error)s", name=tool_name, error=exc,
            ))
        info = {}
    ...
    def get_tools(self, registry=None):
        ...
        if registry not in (None, 'odoo'):
            return tools
        agent = self.env['muk_ai.agent'].sudo().browse(
            self.env.context.get('muk_ai_session_agent_id') or 0
        )
