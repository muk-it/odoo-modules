...
per-record Enterprise context appears inside `<ee_ctx>...</ee_ctx>`.
Empty defaults mean nothing changes for agents that haven't been
...
- **`muk_ai.session` extension** — appends a `<rag>` block to the
  ...
  the latest user message can be embedded, and an `<ee_ctx>` block
  when the session's `view_context` pins a record. Both are no-ops
...
Requires `muk_ai` (Community, ships the chat client and agent
runtime) and the Enterprise `ai` module. The `ai` module in turn
requires the `pgvector` PostgreSQL extension for the RAG features;
when `pgvector` is missing, the RAG step degrades silently to a
no-op and the rest of the bridge keeps working.
...
Dispatch routes through Enterprise's `ir.actions.server._ai_tool_run`
under the calling user's environment, so Enterprise's access checks
and audit hooks all run unchanged. The MuK AI session's
`muk_mcp.log` records the call exactly like it would for a native
MCP tool — including arguments, response, status and timing.
...
dimensions are read from Enterprise (`ai.agent._get_embedding_model`
and `ai.embedding._get_dimensions`), so changes on the Enterprise
side flow through immediately.
...
### How per-record context works

When the MuK AI chat opens on a record, the session's `view_context`
ends up with `kind='record'` plus the model and id. The bridge
intercepts that, calls `record._ai_initialise_context('mail_composer',
None, None)` (the same hook Enterprise uses for its mail composer
flow, which returns the record's serialised field values plus any
model-specific augmentation) and stores the result in
...
silently. Records inheriting `mail.thread` get the chatter history
mixed in for free, courtesy of Enterprise's existing
`mail.thread._ai_initialise_context` override.

## Coexistence

The bridge depends hard on `muk_ai` and `ai` but on no other
Enterprise `ai_*` satellite. Optional satellites — `ai_documents`,
`ai_livechat`, `ai_fields`, `ai_server_actions`, `ai_knowledge`,
`ai_website`, `ai_crm`, etc. — simply contribute more `ai.topic` and
`ai.agent.source` records that admins can then pick from the new
*Enterprise* notebook page. Nothing else needs to change.
...
extending Odoo. We develop custom solutions for your individual
needs to help you focus on your strength and expertise to grow your
business.
