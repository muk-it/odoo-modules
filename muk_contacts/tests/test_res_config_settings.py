...
class TestResConfigSettings(TransactionCase):
    ...
    def test_the_sequence_button_opens_the_contact_number_sequence(self):
        sequence = self.env.ref('muk_contacts.sequence_contact_number')
        settings = self.env['res.config.settings'].create({})
        action = settings.action_open_contact_number_sequence()
        self.assertEqual(action['res_model'], 'ir.sequence')
        self.assertEqual(action['res_id'], sequence.id)
