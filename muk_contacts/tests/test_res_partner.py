...
class TestResPartner(TransactionCase):
    ...
    def test_web_name_search_annotates_the_partner_icons(self):
        company = self.env['res.partner'].create(
            {'name': 'Zeta Icon Co', 'company_type': 'company'}
        )
        delivery = self.env['res.partner'].create(
            {
                'name': 'Zeta Icon Delivery',
                'parent_id': company.id,
                'type': 'delivery',
            }
        )
        result = self.env['res.partner'].web_name_search(
            'Zeta Icon', {'display_name': {}}
        )
        rows = {row['id']: row for row in result}
        self.assertEqual(rows[company.id]['company_type'], 'company')
        self.assertEqual(rows[company.id]['type'], 'contact')
        self.assertEqual(rows[delivery.id]['type'], 'delivery')
