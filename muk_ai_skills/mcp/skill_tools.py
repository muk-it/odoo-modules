...
class SkillToolsMixin(models.AbstractModel):
    ...
    def _resolve_skill_session(self):
        if not (session_id := self.env.context.get('muk_mcp_session_id')):
            raise UserError(_(
                "Skill tools can only be invoked from inside an AI session."
            ))
        session = self.env['muk_ai.session'].sudo().browse(
            session_id
        )
        if not session.exists():
            raise UserError(_(
                "Session %(sid)s no longer exists.", sid=session_id,
            ))
        return session

    def _resolve_visible_skill(self, session, name):
        skill = self.env['muk_ai.skill'].sudo().search(
            [('name', '=', name)], limit=1
        )
    ...
    def _mcp_invoke_skill(self, skill_name=None):
        session = self._resolve_skill_session()
        skill = self._resolve_visible_skill(
            session, skill_name
        )
        return {
            'name': skill.name,
