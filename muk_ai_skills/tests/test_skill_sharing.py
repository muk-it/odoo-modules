...
class TestSkillSharing(TransactionCase):
    ...
    def test_shared_user_reads_but_only_the_owner_writes(self):
        ...
        skill.with_user(self.user_owner).check_access_rights('write')
        skill.with_user(self.user_owner).check_access_rule('write')
        skill.with_user(self.user_other).check_access_rights('read')
        skill.with_user(self.user_other).check_access_rule('read')
        with self.assertRaises(AccessError):
            skill.with_user(self.user_other).check_access_rule('write')
