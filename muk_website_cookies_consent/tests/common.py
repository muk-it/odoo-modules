...
class FakeGeoIP(dict):
    """Stand in for the GeoIP lookup a real request carries.

    A mapping, because that is what a request carries on this branch.
    """

    def __init__(self) -> None:
        """Answer the country the region rules are matched against."""
        super().__init__(country_code='AT')
...
class FakeHttpRequest:
    ...
    def __init__(
        self, headers: dict | None = None, cookies: dict | None = None
    ) -> None:
        """Keep the headers and cookies the gating is meant to see."""
...
class FakeRequest:
    ...
    def __init__(
        ...
        cookies = {CONSENT_COOKIE: cookie_value} if cookie_value else {}
        self.httprequest = FakeHttpRequest(headers, cookies)
