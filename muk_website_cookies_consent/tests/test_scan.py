...
@tagged('post_install', '-at_install')
class TestScanLive(HttpCase):
    """Scanning the running site over HTTP, as the cron does."""
    ...
    def setUpClass(cls) -> None:
        """Point the site at the test server and keep the crawl short."""
        super().setUpClass()
        cls.website = cls.env['website'].search([], limit=1)
        cls.website.write({'cookies_bar': True, 'cookie_scan_pages': 2})

    # ----------------------------------------------------------
    # Tests
    # ----------------------------------------------------------

    def test_the_scan_fetches_the_real_site(self):
        ...
        self.assertTrue(
            result['pages'],
            'A scan that cannot fetch the very site it runs on reports nothing '
            'and would silently leave the registry unchecked.',
        )
        self.assertEqual(self.website.cookie_scan_count, result['pages'])
        self.assertTrue(
            self.env['muk_website_cookies_consent.observation'].search_count(
                [('website_id', '=', self.website.id)]
            ),
            'Odoo sets its own cookies on any page, so a real scan always has '
            'something to file.',
        )
