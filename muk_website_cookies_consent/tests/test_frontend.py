...
class TestFrontend(HttpCase):
    ...
    def test_the_way_back_follows_the_settings_the_editor_writes(self):
        self.website.write(
            {'cookie_reopen_footer': True, 'cookie_reopen_float': 'left'}
        )
        body = self.open_page('/')
        self.assertIn('mk_cookies_footer', body)
        self.assertIn('mk_cookies_float_left', body)
        self.website.write(
            {'cookie_reopen_footer': False, 'cookie_reopen_float': 'none'}
        )
        body = self.open_page('/')
        self.assertNotIn(
            'mk_cookies_footer',
            body,
            'A visitor is still served a way back the editor turned off.',
        )
        self.assertNotIn(
            'mk_cookies_float',
            body,
            'A visitor is still served a button the editor took away.',
        )
