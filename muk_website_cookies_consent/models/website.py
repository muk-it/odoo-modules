...
class Website(models.Model):
    ...
    # Left unannotated: Odoo 16 builds the cache key function by evaluating
    # the signature as source, which a type hint makes unparseable.
    @tools.ormcache('self.id')
    def _get_cookie_category_ids(self):
    ...
    @tools.ormcache('self.id')
    def _get_populated_cookie_category_ids(self):
    ...
    @tools.ormcache('self.id')
    def _get_cookie_service_ids(self):
    ...
    @tools.ormcache('self.id')
    def _get_cookie_declaration_ids(self):
    ...
    @tools.ormcache('self.id')
    def _get_cookie_registry_hash(self):
    ...
    def _clear_cookie_registry_cache(self) -> None:
        ...
        The fingerprint is memoised and the compiled templates it is baked
        into are memoised beside it, so one sweep of the registry cache takes
        both.
        """
    ...
    def _get_cookie_state(self) -> dict | None:
        ...
        if not request:
            return None
        return parse_state(request.httprequest.cookies.get(CONSENT_COOKIE))
    ...
    def _get_blocked_iframe_containers_classes(self) -> frozenset:
        """Return the classes of containers that build an embed client-side.

        Core carries this list from Odoo 18 on; on this branch the module has
        to name them itself, because an element that will grow an iframe later
        has no source to judge yet.
        """
        return BLOCKED_IFRAME_CLASSES

    def _get_blocked_third_party_domains_list(self) -> list[str]:
        """Return the hosts the client-side watcher must still block.

        The list is the hosts of the services that are actually still refused,
        plus the shipped defaults that no service claims — those stay blocked
        until nothing is refused, since nothing says which purpose they belong
        to. The hosts Consent Mode governs are the exception, and they have to
        be exempt on exactly the terms the server strips by: the watcher
        rewrites the ``src`` of a script created at runtime, which is how a Tag
        Manager container loads, so leaving them here would block the very tag
        this module signals to.
        """
        ...
        if not self._allConsentsGranted():
            ...
            blocked.update(set(BLOCKED_THIRD_PARTY_HOSTS) - claimed)
    ...
    def _control_third_party_trackers_in_html(self, html_content: str) -> Markup:
        ...
        This branch renders the custom head and footer code verbatim, so a tag
        pasted there is the one place blocking cannot reach. The template
        routes both fields through here instead, which is what Odoo 19 does in
        core.
    ...
    def _should_remove_third_party_trackers(self) -> bool:
        ...
        Nothing on this branch strips a third-party tag, so the whole answer
        belongs to the registry: stripping runs while a declared service is
        refused, and stops for that service as soon as its own purpose is
        granted. It also stays on while anything is refused, to cover the
        shipped default hosts that no service claims a purpose for.
        """
        if not self._is_cookie_consent_active() or not self.cookie_blocking:
            return False
    ...
    def _is_tag_host_blocked(self, tagName: str, atts: dict) -> bool:  # noqa: N803
        """Report whether an element's host sits on the shipped block list.

        A bare domain covers its subdomains and its ``www`` form, but not a
        ...
        if not src_host:
            return False
        return any(
            src_host == host.removeprefix('www.')
            or src_host.endswith('.' + host.removeprefix('www.'))
            for host in self._get_blocked_third_party_domains_list()
        )
    ...
    def _remove_third_party_trackers(
        ...
        The watchlist split is the one Odoo 19 uses: ``domains`` judges the
        element's own source, ``classes`` judges a container an iframe may be
        built inside later, which is why that case stops at the marker. The
        stamp is what lets the browser bring back one service when its purpose
        is granted, instead of requiring blanket consent.
        """
        checkers = {
