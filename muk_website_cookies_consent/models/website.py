...
class Website(models.Model):
    ...
    def _control_third_party_trackers_in_html(self, html_content: str) -> Markup:
        ...
        Core parses the value as a document, which only holds when it has
        exactly one root element. What an admin pastes into the custom head or
        footer code is a snippet — typically a comment in front of a tag — and
        a document parse wraps it in ``<html><head>`` and drops the comment.
        Injected into the real head, that closes it early and the browser
        recovers the rest of the page as body content. Wrapping it keeps the
