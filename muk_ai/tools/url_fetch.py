...
def _validate_url(url: str) -> tuple[str, list[str]]:
    ...
    if parsed.scheme != 'https':
        raise UserError(_('@url: only accepts https:// URLs.'))
    if not (host := parsed.hostname):
        raise UserError(_('@url: missing hostname.'))
    try:
        infos = socket.getaddrinfo(host, None)
    except socket.gaierror as exc:
        raise UserError(_('@url: DNS lookup failed for %s: %s', host, exc)) from exc
    resolved = []
    for info in infos:
...
def fetch_url(url: str) -> FetchResult:
    ...
    for _attempt in range(MAX_REDIRECTS + 1):
        ...
        try:
            ...
            try:
                if response.status in _REDIRECT_STATUSES:
                    if not (location := response.headers.get('Location')):
                        raise UserError(
                            _('@url: redirect from %s had no Location.', current)
                        )
            ...
            pool.close()
    raise UserError(_('@url: too many redirects for %s.', url))
