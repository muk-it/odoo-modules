...
class SearchBackend:
    ...
    def search(self, query: SearchQuery) -> list[SearchHit]:
        ...
        if self.needs_key and not self.api_key:
            raise UserError(_lt('Web search backend %s needs an API key.', self.label))
        if self.needs_url and not self.url:
            raise UserError(_lt('Web search backend %s needs a URL.', self.label))
        try:
            ...
            raise UserError(
                _lt('Web search (%s) failed: %s', self.label, detail[:500])
            ) from error
        except (requests.RequestException, ValueError) as error:
            raise UserError(
                _lt('Web search (%s) failed: %s', self.label, error)
            ) from error
