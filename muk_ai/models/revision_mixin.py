...
class AIRevisionMixin(models.AbstractModel):
    ...
    def _prompt_history_entries(self, field_name):
    ...
    def prompt_history_get_content(self, field_name, index):
    ...
    def prompt_history_unified_diff(self, field_name, index):
    ...
    def prompt_history_restore(self, field_name, index):
        ...
        return {
            ...
            'params': {
                ...
                'message': _(
                    ...
                    field=field_label, index=index + 1,
                ),
    ...
    def action_open_prompt_history(self, field_name=None):
    ...
    def _compute_prompt_history_metadata(self):
        for record in self:
    ...
    @api.depends('prompt_history')
    def _compute_prompt_history_count(self):
        for record in self:
            history = record.prompt_history or {}
    ...
    def write(self, vals):
        ...
        affected = set(vals).intersection(
            self._get_prompt_fields()
        )
        if not affected:
        ...
        for record in self:
            ...
            for name in affected:
                ...
                if old and old != (record[name] or ''):
                    ...
                    changed = True
            if changed:
                super(AIRevisionMixin, record).write({
