...
class AISpace(models.Model):
    ...
    def _unclaimed_session_domain(self) -> list:
        ...
        for space in self.search([('domain', '!=', False)]):
            domain = expression.AND(
                [domain, ['!'] + expression.normalize_domain(space._parsed_domain())]
            )
        return domain
    ...
    def _count_by_space(self, session_ids: list[int] | None = None) -> dict[int, int]:
        ...
        count of its own, as each carries a different domain. Odoo 16 groups
        through ``read_group``, which answers with dictionaries keyed by the
        grouped field rather than with recordset and aggregate pairs.
        ...
        counts = {
            group['space_id'][0]: group['space_id_count']
            for group in sessions.read_group(
    ...
    def _check_domain(self) -> None:
        ...
        Odoo 16 has no ``Domain`` class, so the domain is compiled against the
        session model instead: parsing rejects a malformed literal and building
        the query rejects both a value that is no domain at all and a leaf
        naming a field the model does not have.
        """
        sessions = self.env['muk_ai.session']
