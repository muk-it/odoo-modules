...
const SESSION_RECORD = {
    ...
    pending_ask: null, view_context: null, last_text: '', error_message: null,
    iteration_count: 1, total_input_tokens: 42, total_output_tokens: 24,
    last_input_tokens: 10, context_window: 8000, total_cost: 0.12,
    agent_id: [3, 'Helper'],
    override_approval_mode: false, effective_approval_mode: 'ask',
};
...
function makeHarness(options = {}) {
    ...
    class Harness extends Component {
        ...
        setup() { session = useAiSession(options); }
    }
...
async function mountAndLoad(harness, sessionId = 7) {
    ...
    if (sessionId !== null) { await session.load(sessionId); }
    return session;
}
...
test('tool_call_start registers a streaming tool and log kind tool_call clears it', async () => {
    ...
    bus.emit({
        session_id: 7,
        type: 'log',
        payload: { kind: 'tool_call', name: 'search_read', arguments: {}, call_id: 'c1' },
    });
...
test('onStop surfaces errors via notification', async () => {
    onRpc('muk_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('muk_ai.session', 'action_stop', () => { throw new Error('stop-err'); });
    makeBusMock();
    const notifications = [];
...
test('loadAgents swallows errors and resets agents to empty', async () => {
    onRpc('muk_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('muk_ai.agent', 'search_read', () => { throw new Error('down'); });
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
    await session.loadAgents();
...
test('runUnpin warns when no view context is pinned', async () => {
    ...
    const notifications = [];
    mockService('notification', { add: (msg, opts) => notifications.push({ msg: String(msg), type: opts?.type }) });
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('bus tool_call_args_delta accumulates on the matching streaming tool', async () => {
    ...
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
    bus.emit({ session_id: 7, type: 'tool_call_start', payload: { call_id: 'c9', name: 't' } });
    bus.emit({ session_id: 7, type: 'tool_call_args_delta', payload: { call_id: 'c9', delta: '{"a":' } });
    bus.emit({ session_id: 7, type: 'tool_call_args_delta', payload: { call_id: 'c9', delta: '1}' } });
...
test('onAttachFiles is a noop when files array is empty', async () => {
    onRpc('muk_ai.session', 'read', () => [SESSION_RECORD]);
    let attempts = 0;
    onRpc('muk_ai.session', 'upload_attachments', () => { attempts++; return []; });
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
    await session.onAttachFiles([]);
...
test('onRemoveAttachment filters the attachment and calls discard RPC', async () => {
    ...
    session.state.pendingAttachments = [
        { id: 1, filename: 'a' }, { id: 2, filename: 'b' },
    ];
...
test('copyText writes to clipboard when navigator.clipboard is available', async () => {
    ...
    try {
        ...
        Object.defineProperty(navigator, 'clipboard', {
            configurable: true, value: originalClipboard,
        });
    }
});
...
test('onSend is a noop while session is running', async () => {
    onRpc('muk_ai.session', 'read', () => [{ ...SESSION_RECORD, state: 'running' }]);
    let attempts = 0;
    onRpc('muk_ai.session', 'send_message', () => { attempts++; return SNAPSHOT_RUNNING; });
    onRpc('muk_ai.session', 'start', () => { attempts++; return SNAPSHOT_RUNNING; });
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
    session.onInputChange('hi');
...
test('setApprovalMode swallows errors via notification', async () => {
    onRpc('muk_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('muk_ai.session', 'set_approval_mode', () => { throw new Error('boom'); });
    const notifications = [];
...
test('approveTool swallows RPC errors via notification', async () => {
    onRpc('muk_ai.session', 'read', () => [{
        ...SESSION_RECORD, state: 'waiting',
        pending_ask: { kind: 'approval', call_id: 'c1' },
    }]);
    onRpc('muk_ai.session', 'approve_tool', () => { throw new Error('bad'); });
    const notifications = [];
    mockService('notification', { add: (msg) => notifications.push(String(msg)) });
    makeBusMock();
    const harness = makeHarness();
...
test('openPinnedContext surfaces action errors via notification', async () => {
    ...
    const notifications = [];
    mockService('action', { doAction: () => { throw new Error('no route'); } });
    mockService('notification', { add: (msg) => notifications.push(String(msg)) });
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('onAttachFiles surfaces upload errors via notification', async () => {
    onRpc('muk_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('muk_ai.session', 'upload_attachments', () => { throw new Error('disk full'); });
    const notifications = [];
    mockService('notification', { add: (msg) => notifications.push(String(msg)) });
    makeBusMock();
    const harness = makeHarness();
...
test('/clear while running is a noop (onSend bails before slash dispatch)', async () => {
    ...
    const dialogs = [];
    mockService('dialog', {
        add: (Component, props) => { dialogs.push(props); return () => {}; },
    });
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
