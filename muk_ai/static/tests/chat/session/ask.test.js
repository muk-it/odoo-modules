...
test('askArgsText pretty-prints preview.arguments JSON', () => {
    const text = askArgsText({ preview: { arguments: { ids: [1, 2], model: 'res.partner' } } });
    expect(text).toBe('{\n  "ids": [\n    1,\n    2\n  ],\n  "model": "res.partner"\n}');
});
...
test('askViewMode defaults to human when preview.kind exists', () => {
    expect(askViewMode({ callId: 'c1', preview: { kind: 'delete' } }, {})).toBe('human');
});
...
test('askViewMode handles missing block / overrides', () => {
    ...
    expect(askViewMode({ callId: 'c1', preview: { kind: 'delete' } }, null)).toBe('human');
});
