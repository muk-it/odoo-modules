...
test('viewContextLabel renders a record with display_name', () => {
    expect(viewContextLabel({
        kind: 'record', model: 'res.partner', display_name: 'Acme', id: 12,
...
test('viewContextLabel returns model only when neither display_name nor id', () => {
    expect(viewContextLabel({ kind: 'record', model: 'res.partner' })).toBe('res.partner');
});
...
test('viewContextLabel defaults list view_type to "list"', () => {
    expect(viewContextLabel({ kind: 'list', model: 'sale.order' })).toBe('sale.order · list');
});
...
test('viewContextTooltip combines label and hint', () => {
    const tip = String(viewContextTooltip({
        kind: 'record', model: 'res.partner', display_name: 'Acme',
    }));
...
test('viewContextTooltip injects domain JSON only for non-empty list domains', () => {
    const withDomain = String(viewContextTooltip({
        kind: 'list', model: 'sale.order',
        domain: [['state', '=', 'sale']],
    }));
    ...
    const empty = String(viewContextTooltip({
        kind: 'list', model: 'sale.order', domain: [],
    }));
    expect(empty.split('\n').length).toBe(2);
    const recordTip = String(viewContextTooltip({
        kind: 'record', model: 'res.partner', domain: [['x', '=', 1]],
    }));
