...
test('toFileModel returns the same instance when given an existing AIAttachment', () => {
    const first = toFileModel({ id: 1, filename: 'a.png', mimetype: 'image/png', size: 1 });
...
test('isText recognizes csv and markdown in addition to FileModelMixin defaults', () => {
    const csv = toFileModel({ id: 1, filename: 'x.csv', mimetype: 'text/csv', size: 1 });
    const md = toFileModel({ id: 2, filename: 'x.md', mimetype: 'text/markdown', size: 1 });
    const png = toFileModel({ id: 3, filename: 'x.png', mimetype: 'image/png', size: 1 });
...
test('isText still honors FileModelMixin defaults (text/plain)', () => {
    const txt = toFileModel({ id: 1, filename: 'x.txt', mimetype: 'text/plain', size: 1 });
