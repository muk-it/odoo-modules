...
import '@muk_ai/views/fields/module_link/module_link';

describe.current.tags('muk_ai');
defineMailModels();


class MukAiModuleLinkModel extends models.Model {
    _name = 'muk_ai.module_link_model';
...
defineModels([MukAiModuleLinkModel, IrModuleModule]);
...
test('ModuleLinkField renders a checkbox when the module is on disk', async () => {
    await mountView({
        resModel: 'muk_ai.module_link_model',
        resId: 1,
        type: 'form',
        arch: ARCH,
    });
