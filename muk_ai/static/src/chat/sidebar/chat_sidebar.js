...
// Odoo 16 keeps the sortable hook at ``@web/core/utils/sortable``; the OWL
// rewrite moved to ``sortable_owl`` in 17.0.
...
export class ChatSidebar extends Component {
    ...
    setup() {
        ...
        // Space the dragged chat is over. Kept beside the rendered highlight
        // because Odoo 16 calls ``onDragEnd`` before ``onDrop`` — the reverse
        // of 17.0 — so the drop would otherwise read a target already cleared.
        ...
        useSortable({
            ...
            connectGroups: true,
            cursor: 'grabbing',
            // Odoo 16 has no ``placeholderClasses``: it inserts a shallow clone
            // of the dragged row right after it and only hides it with an
            // inline ``visibility``, leaving a gap that contradicts the
            // highlighted space. Marking that clone lets the stylesheet drop it.
            onDragStart: ({ element }) => {
                element.nextElementSibling?.classList.add('mk_drag_placeholder');
            },
            onGroupEnter: ({ group }) => {
                this._dropTarget = group.dataset.spaceId || 'loose';
                this.state.dropTargetId = this._dropTarget;
            },
            onGroupLeave: () => {
                this._dropTarget = null;
