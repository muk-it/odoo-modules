...
class AIWindow(models.AbstractModel):
    ...
    @api.model
    def _view_types(self):
        return [
            code
            for code, _label in self.env['ir.ui.view']._fields['type'].selection
        ]

    @api.model
    def _window_targets(self):
        return [
            code
            for code, _label in self.env['ir.actions.act_window']._fields['target'].selection
        ]

    @api.model
    def _resolve_window_action(self, action_ref):
        if isinstance(action_ref, int):
            return self.env['ir.actions.actions'].sudo().browse(
                action_ref,
            )
        if (
            isinstance(action_ref, str) and
            (ref := action_ref.strip()) and
            '.' in ref
        ):
    ...
    def _mcp_open_record(
        self, model, res_id, view_type='form', target='current',
    ):
