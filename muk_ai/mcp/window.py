...
class AIWindow(models.AbstractModel):
    ...
    def _check_view_type(self, view_type) -> str:
        """Validate and normalize a view type, mapping ``list`` to ``tree``.
    ...
    def _mcp_open_record(
        ...
        target_model.browse(int(res_id)).check_access_rights('read')
        target_model.browse(int(res_id)).check_access_rule('read')
            ...
            'properties': {
                ...
                'view_type': {
                    ...
                    'description': (
                        ...
                        '(e.g. "tree", "kanban", "pivot"). Default: "tree".'
                    ),
                    'default': 'tree',
                },
                'domain': {
