...
def is_hidden(node: etree._Element) -> bool:
    """Say whether a field node is hidden, however the release spells it.

    ``get_view`` pops ``invisible`` off the node on this branch and folds it
    into the compiled ``modifiers`` attribute, so the raw attribute alone
    reports every hidden field as a visible one.
    """
    if node.get('invisible') == '1':
        return True
    modifiers = node.get('modifiers')
    return bool(modifiers) and json.loads(modifiers).get('invisible') is True
...
def ability_rows(env: Environment) -> list[str]:
    ...
    for node in group:
        ...
        elif node.tag == 'field' and not is_hidden(node):
