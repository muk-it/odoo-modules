...
class Partner(models.Model):
    ...
    def _is_own_mobile(self, number: str) -> bool:
        """Return whether ``number`` would only copy the mobile into an empty phone."""
        self.ensure_one()
        if self.phone or not self.mobile:
            return False
        sanitized = self._phone_format(number=number) or number
        return sanitized in (self.mobile_sanitized, self.mobile)

    def _has_name_parts(self) -> bool:
        """Return whether the partner is a person whose name is kept in parts."""
        self.ensure_one()
        return not self.is_company and self.type == 'contact'

    def _split_name_into_parts(self) -> None:
        """Store the name of each partner as first and last name, or whole."""
        for record in self:
            lastname, firstname = self._split_name(
                (record.name or '').strip(), not record._has_name_parts()
            )
    ...
    def _honorific_shortcuts(self, fname: str) -> list[str]:
        """Return the shortcuts of an honorific field ordered by their sequence.

        An unsaved many2many keeps the order its commands were written in,
        so the formatted name and the vCard sort by sequence here.
        """
    ...
    def _build_vcard(self) -> web_partner.VComponentProxy:
        """Enrich the base vCard with extra contact and address details."""
        vcard = super()._build_vcard()

        def get_vcard_content_element(name: str) -> web_partner.VBaseProxy:
            """Return the first line of a vCard property, adding it if missing."""
            lines = vcard.contents.get(name)
            return lines[0] if lines else vcard.add(name)
        ...
        kind = vcard.add('kind')
        kind.value = 'org' if self.is_company else 'individual'
        if self.child_ids:
    ...
    @api.depends(lambda self: ['phone', *self._phone_get_sanitize_triggers()])
    def _compute_phone_formatted(self) -> None:
        """Format the phone itself, while the mobile stays the primary number."""
        for record in self:
            record.phone_formatted = record._phone_get_formatted(
                record._phone_format(fname='phone')
            )

    @api.depends(lambda self: ['mobile', *self._phone_get_sanitize_triggers()])
    def _compute_mobile_companion_fields(self) -> None:
        """Give the mobile its own dial number and display format."""
        self._phone_update_companion_fields(('mobile',))

    @api.depends('phone_sanitized', 'phone', 'mobile')
    def _compute_blacklisted(self) -> None:
        """Flag the phone and the mobile separately when the primary is blacklisted."""
        super()._compute_blacklisted()
        for record in self:
            blacklisted = record.phone_sanitized_blacklisted
            record.phone_blacklisted = (
                blacklisted
                and record._phone_format(fname='phone') == record.phone_sanitized
            )
            record.mobile_blacklisted = (
                blacklisted and record.mobile_sanitized == record.phone_sanitized
            )
    ...
    def write(self, vals: dict) -> bool:
        """Split the name again when a partner turns into or out of a person.

        A phone that only repeats the partner's own mobile is not stored.
        """
        if vals.get('phone'):
            copies = self.filtered(
                lambda partner: partner._is_own_mobile(vals['phone'])
            )
            if copies:
                ...
                if rest:
                    copies.write(rest)
                others = self - copies
                return others.write(vals) if others else True
        res = super().write(vals)
        if 'name' not in vals and {'is_company', 'type'} & set(vals):
            self._split_name_into_parts()
        return res
