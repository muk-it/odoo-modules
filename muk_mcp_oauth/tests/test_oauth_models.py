...
class TestOAuthModels(TransactionCase):
    ...
    def test_authenticate_regular_key_untouched(self):
        # The key owner must be an active user: authenticate() rejects keys
        # owned by archived users (like the superuser running the tests).
        data = self.env["muk_mcp.key"].with_user(self.user).generate_playground_key(name="Plain", scope="write")
